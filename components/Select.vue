<template>
  <Button
    classes="flex justify-between items-center gap-5 bg-white text-dark-grey hover:bg-white rounded-none select-btn"
    @click="toggleSelect">
    <template #default>
      <slot>
      ...
      <ChevronDown class="transition-all chevron" :class="{ opened: isOpened }" />
    </template>
  </Button>
  <Transition name="slide">
    <KeepAlive>
      <ul v-if="isOpened" class="flex flex-col rounded-lg border border-grey overflow-hidden select-list">
